...
export const registerReturnEntry = async (transaction: Transaction, originalSaleId: number) => {
  const entry = {
    ...
    type: 'egreso',
    category: 'devolucion',
    concept: `Devolución de venta #${originalSaleId}`,
...
export const registerDebtPaymentEntry = async (transaction: Transaction, client: any) => {
  const entry = {
    ...
    concept: `Cobro de deuda - Cliente: ${client.name}`,
    description: `Abono a cuenta - Método: ${transaction.payMethod?.replace('_', ' ') || 'EFECTIVO'}`,
    amount: transaction.total,
    referenceId: transaction.id,
...
export const registerCreditEntry = async (transaction: Transaction, client: any) => {
  const entry = {
    ...
    concept: `Venta a crédito #${transaction.id}`,
