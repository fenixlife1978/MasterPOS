...
// ✅ Función auxiliar para formatear fecha YYYY-MM-DD a DD/MM/YYYY sin desfase horario
function formatDateFromString(dateStr: string): string {
  // Si ya tiene formato numérico (timestamp), usar el método tradicional
...
// ✅ Generador de ID único para pagos y asientos (devuelve número)
...
export default function SuppliersModule() {
  ...
  // ❌ Eliminado estado global expandedInvoice
  ...
  // Cargar pagos desde Firestore (la suscripción es la única fuente de verdad)
  useEffect(() => {
  ...
  // Distribuir pago global entre facturas pendientes (orden cronológico ascendente)
  const distributePayment = async (
    ...
    for (const invoice of pendingInvoices) {
      ...
      const newPayment: SupplierPayment = {
        ...
        reference: reference || '',
        bank: bank || '',
        notes: `Pago parcial distribuido desde abono global. Tasa usada: ${formatBsNumber(exchangeRate)} Bs/USD`
      };
    ...
    try {
      ...
      if (selectedInvoiceForPayment) {
        ...
        const newPayment: SupplierPayment = {
          ...
          notes: `Pago registrado. Tasa usada: ${formatBsNumber(exchangeRateUsed)} Bs/USD`
        };
        await syncService.saveSupplierPayment(newPayment);
        ...
        toast({ 
          ...
          description: `Se ha registrado un pago de ${formatUsd(payAmount)} a ${selectedSupplierForPayment.name}` 
        });
      } else {
        ...
        toast({ 
          title: "Pago global aplicado", 
          description: `Se ha distribuido ${formatUsd(paymentData.amount)} entre las facturas pendientes de ${selectedSupplierForPayment.name}` 
        });
      }
  ...
  const handleReversePayment = useCallback(async (payment: SupplierPayment) => {
    ...
    try {
      ...
      await syncService.saveAccountingEntry({
        ...
        description: `Reversión de pago de ${formatUsd(payment.amount)} USD (tasa actual ${formatBsNumber(state.exchangeRate)}) = ${formatBs(amountBs)}`,
        amount: amountBs,
  ...
  // ✅ PurchaseHistoryModal con estado de expansión local (corrige los destellos)
  const PurchaseHistoryModal = ({ supplier, onClose }: { supplier: Supplier; onClose: () => void }) => {
    // Estado local para expandir/colapsar facturas (ya no es global)
    const [localExpandedInvoice, setLocalExpandedInvoice] = useState<number | null>(null);
    ...
    const toggleInvoiceExpand = (invoiceId: number) => {
      setLocalExpandedInvoice(prev => prev === invoiceId ? null : invoiceId);
    };

    const getInvoicePayments = (invoiceId: number): SupplierPayment[] => {
      return supplierPayments.filter(p => p.invoiceId === invoiceId).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    };
    ...
    return (
      <Dialog open={true} onOpenChange={onClose}>
        <DialogContent className="bg-white border border-[#9E9E9E] text-black max-w-4xl p-0 overflow-hidden rounded-xl shadow-xl max-h-[90vh] flex flex-col">
          <DialogHeader className="sr-only"><DialogTitle>Historial de Compras</DialogTitle></DialogHeader>
          <div className="bg-[#1A2C4E] p-4 text-white flex-shrink-0">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-primary/20 rounded-lg flex items-center justify-center"><History size={20} className="text-primary" /></div>
                <div><h3 className="text-base font-black">Historial de Compras</h3><p className="text-xs opacity-70">{supplier.name} • {supplier.rif}</p></div>
              </div>
          ...
          <div className="flex-1 overflow-y-auto p-4">
            <div className="grid grid-cols-3 gap-3 mb-4">
              <div className="bg-slate-50 rounded-lg p-3 text-center border"><p className="text-[9px] font-black uppercase text-slate-500">Total Facturado</p><p className="text-xl font-black text-black">{formatUsd(totalPurchases)}</p></div>
              <div className="bg-slate-50 rounded-lg p-3 text-center border"><p className="text-[9px] font-black uppercase text-slate-500">Total Abonado</p><p className="text-xl font-black text-green-600">{formatUsd(totalPaid)}</p></div>
              <div className="bg-slate-50 rounded-lg p-3 text-center border"><p className="text-[9px] font-black uppercase text-slate-500">Saldo por Pagar</p><p className={cn("text-xl font-black", totalDebt > 0 ? "text-red-600" : "text-green-600")}>{formatUsd(totalDebt)}</p></div>
            </div>
            {totalDebt > 0 && (
              <div className="mb-4 flex justify-end">
                <Button onClick={() => handleOpenPaymentModal(supplier)} className="bg-green-600 hover:bg-green-700 text-white font-black h-8 text-xs"><HandCoins size={14} className="mr-1" /> PAGAR DEUDA ({formatUsd(totalDebt)})</Button>
              </div>
            )}
            <div className="space-y-2">
              {supplierInvoices.length === 0 ? (
                <div className="text-center py-12 text-black/30 italic">No se han registrado compras para este proveedor</div>
              ) : (
                supplierInvoices.map(inv => {
                  const isExpanded = localExpandedInvoice === inv.id;
                    ...
                    <div key={inv.id} className="border border-[#9E9E9E] rounded-lg overflow-hidden">
                      ...
                      >
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-[9px]">Pagado: {formatUsd(inv.paidAmount)}</p>
                          {owed > 0 && (
                            <button
                      ...
                      {isExpanded && (
                        <div className="bg-[#FAFAFA] p-3 border-t border-[#9E9E9E] animate-in slide-in-from-top-1 duration-200">
                          <h4 className="text-[9px] font-black uppercase text-black/60 mb-2 flex items-center gap-1"><Package size={10} /> Detalle de Productos</h4>
                          ...
                          {invoicePayments.length > 0 && (
                            <div>
                              <h5 className="text-[8px] font-black uppercase text-black/60 mb-1">Historial de Abonos</h5>
                              <div className="space-y-1 max-h-32 overflow-y-auto">
                                {invoicePayments.map(payment => (
                                  <div key={payment.id} className="flex justify-between items-center text-[9px] bg-white p-2 rounded border border-gray-200">
                                    <div>
                                      <p className="font-mono text-[10px]">{formatDateFromString(payment.date)}</p>
                                      <p className="text-[8px] text-black/50">{payment.method}</p>
                                    </div>
                                    <div className="text-right">
                                      <p className="font-bold text-green-600">{formatUsd(payment.amount)}</p>
                                    </div>
                                  </div>
                                ))}
                              </div>
          ...
          <div className="bg-slate-50 p-3 border-t flex justify-between items-center flex-shrink-0">
            <Button onClick={() => setViewingPayments(supplier)} variant="outline" size="sm" className="h-7 text-xs border-[#9E9E9E]"><Wallet size={12} className="mr-1" /> VER HISTORIAL DE PAGOS</Button>
  ...
  return (
    <div className="h-full flex flex-col bg-background overflow-hidden">
      <div className="flex justify-between items-center pt-4 px-6 flex-shrink-0">
        <div><h2 className="text-xl font-headline font-black text-black">Gestión de Proveedores</h2><p className="text-xs text-black/50">Directorio de proveedores y auditoría de compras</p></div>
        <Button onClick={() => { resetForm(); setEditingSupplier(null); setIsAdding(true); }} className="bg-primary text-black font-black h-8 text-xs px-3"><Plus size={13} className="mr-1" /> NUEVO PROVEEDOR</Button>
      </div>
      <div className="px-6 mt-3 flex-shrink-0"><div className="relative max-w-sm"><Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-black/40" /><Input placeholder="Buscar por nombre o RIF..." className="pl-9 h-8 border-[#9E9E9E] text-xs" value={search} onChange={(e) => setSearch(e.target.value)} /></div></div>
          ...
          <div className="overflow-y-auto flex-1 scrollbar-thin">
            <Table>
              <TableHeader className="bg-[#E8E8E8] sticky top-0 z-10">
                <TableRow><TableHead className="text-[9px] font-black uppercase">Proveedor</TableHead><TableHead className="text-[9px] font-black uppercase">RIF</TableHead><TableHead className="text-[9px] font-black uppercase">Contacto</TableHead><TableHead className="text-[9px] font-black uppercase text-right">Compras Acum.</TableHead><TableHead className="text-[9px] font-black uppercase text-right">Saldo Deudor</TableHead><TableHead className="text-[9px] font-black uppercase text-center">Acciones</TableHead></TableRow>
              </TableHeader>
              <TableBody>
                {filteredSuppliers.length === 0 ? <TableRow><TableCell colSpan={6} className="text-center py-10 text-black/40 italic">No hay proveedores en el directorio</TableCell></TableRow> : filteredSuppliers.map(s => {
                  ...
                  return (
                    <TableRow key={s.id} className="border-b border-[#9E9E9E]/40 hover:bg-[#F5F5F5]">
                      ...
                      <TableCell><p className="text-[10px] font-medium">{s.contactPerson}</p><p className="text-[9px] text-black/40">{s.phone}</p></TableCell>
                      <TableCell className="text-right font-mono text-xs text-secondary font-bold">{formatUsd(totalPurchases)}</TableCell>
                      <TableCell className="text-right"><span className={cn("font-mono text-xs font-black", debt > 0 ? "text-red-600" : "text-green-600")}>{formatUsd(debt)}</span></TableCell>
                      <TableCell className="text-center">
                        <div className="flex justify-center gap-1">
                          ...
                          {debt > 0 && <button onClick={() => handleOpenPaymentModal(s)} className="h-6 w-6 rounded hover:bg-green-100 text-green-600" title="Pagar Deuda"><HandCoins size={11} /></button>}
      ...
      <Dialog open={isAdding} onOpenChange={(val) => { if(!val) { setIsAdding(false); setEditingSupplier(null); } }}>
        <DialogContent className="bg-white border border-[#9E9E9E] text-black max-w-md p-0 overflow-hidden rounded-2xl shadow-xl">
          ...
          <div className="flex flex-col">
            <div className="bg-[#1A2C4E] p-4 text-white"><div className="flex justify-between items-center"><div className="flex items-center gap-2"><Truck size={20} className="text-primary" /><h3 className="text-lg font-black">{editingSupplier ? 'Editar Proveedor' : 'Nuevo Proveedor'}</h3></div><button onClick={() => setIsAdding(false)} className="text-white/60 hover:text-white"><X size={18} /></button></div></div>
            <form onSubmit={handleSave} className="p-5 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3"><div><label className="text-[10px] font-bold text-black/60 uppercase tracking-widest block mb-1">Teléfono</label><Input value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className="h-9 border-[#9E9E9E]" /></div><div><label className="text-[10px] font-bold text-black/60 uppercase tracking-widest block mb-1">Email</label><Input type="email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="h-9 border-[#9E9E9E]" /></div></div>
              ...
              <div className="bg-[#F5F5F5] -mx-5 -mb-5 p-4 mt-4 border-t flex justify-end gap-3"><Button type="button" variant="ghost" onClick={() => setIsAdding(false)} className="px-4 text-black">CANCELAR</Button><Button type="submit" className="px-6 bg-primary text-black font-black">GUARDAR PROVEEDOR</Button></div>
            </form>
          </div>
        </DialogContent>
      </Dialog>
